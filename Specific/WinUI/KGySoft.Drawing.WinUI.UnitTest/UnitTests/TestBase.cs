...
namespace KGySoft.Drawing.WinUI.UnitTests
{
    public abstract class TestBase
    {
        ...
        protected async Task ExecuteTest(Action callback)
        {
            var result = new TaskCompletionSource<bool>();

            // ReSharper disable once AsyncVoidLambda - not a problem, finished state is signed in result
            if (!Program.Dispatcher.TryEnqueue(DispatcherQueuePriority.Normal, () =>
                {
                    try
                    {
                        callback.Invoke();
        ...
        protected async Task ExecuteTestAsync(Func<Task> callback)
        {
            ...
            // ReSharper disable once AsyncVoidLambda - not a problem, finished state is signed in result
            if (!Program.Dispatcher.TryEnqueue(DispatcherQueuePriority.Normal, async () =>
            {
                    try
                    {
