...
namespace KGySoft.Drawing.Imaging
{
    internal struct CopySession
    {
        ...
        internal void PerformCopyWithDithering(IQuantizingSession quantizingSession, IDitheringSession ditheringSession, bool skipTransparent, in Array2D<byte> mask, Point maskOffset)
        {
            ...
            PerformCopyWithDithering(ditheringSession);
        }
        ...
        internal void PerformCombine(Func<Color32, Color32, Color32> combineFunction, IQuantizer? quantizer, IDitherer? ditherer)
        {
            ...
            if (ditherer != null)
            {
                ...
                // Using a clipped source for quantizer/ditherer if needed (matters only for the ditherer). Note: the actual combining uses the original source for the best performance
                IReadableBitmapData initSource = SourceRectangle.Size == Source.Size
                    ? Source
                    : Source.Clip(SourceRectangle);
            ...
            if (SourceRectangle.Width < parallelThreshold)
            {
                ...
                for (int y = 0; y < SourceRectangle.Height; y++)
                {
                    if (context.IsCancellationRequested)
                        return;
                    for (int x = 0; x < SourceRectangle.Width; x++)
                        rowDst.DoSetColor32(x + TargetRectangle.X, combineFunction.Invoke(rowSrc.DoGetColor32(x + SourceRectangle.X), rowDst.DoGetColor32(x + TargetRectangle.X)));
                    rowSrc.MoveNextRow();
                    rowDst.MoveNextRow();
                    context.Progress?.Increment();
                }
            ...
            void ProcessRow(int y)
            {
                ...
                for (int x = 0; x < width; x++)
                    rowDst.DoSetColor32(x + offsetDst, f.Invoke(rowSrc.DoGetColor32(x + offsetSrc), rowDst.DoGetColor32(x + offsetDst)));
            }
        ...
        internal void PerformCombine(Func<Color64, Color64, Color64> combineFunction)
        {
            ...
            if (SourceRectangle.Width < parallelThreshold)
            {
                ...
                for (int y = 0; y < SourceRectangle.Height; y++)
                {
                    if (context.IsCancellationRequested)
                        return;
                    for (int x = 0; x < SourceRectangle.Width; x++)
                        rowDst.DoSetColor64(x + TargetRectangle.X, combineFunction.Invoke(rowSrc.DoGetColor64(x + SourceRectangle.X), rowDst.DoGetColor64(x + TargetRectangle.X)));
                    rowSrc.MoveNextRow();
                    rowDst.MoveNextRow();
                    context.Progress?.Increment();
                }
            ...
            void ProcessRow(int y)
            {
                ...
                for (int x = 0; x < width; x++)
                    rowDst.DoSetColor64(x + offsetDst, f.Invoke(rowSrc.DoGetColor64(x + offsetSrc), rowDst.DoGetColor64(x + offsetDst)));
            }
        ...
        internal void PerformCombine(Func<ColorF, ColorF, ColorF> combineFunction)
        {
            ...
            if (SourceRectangle.Width < parallelThreshold)
            {
                ...
                for (int y = 0; y < SourceRectangle.Height; y++)
                {
                    if (context.IsCancellationRequested)
                        return;
                    for (int x = 0; x < SourceRectangle.Width; x++)
                        rowDst.DoSetColorF(x + TargetRectangle.X, combineFunction.Invoke(rowSrc.DoGetColorF(x + SourceRectangle.X), rowDst.DoGetColorF(x + TargetRectangle.X)));
                    rowSrc.MoveNextRow();
                    rowDst.MoveNextRow();
                    context.Progress?.Increment();
                }
            ...
            void ProcessRow(int y)
            {
                ...
                for (int x = 0; x < width; x++)
                    rowDst.DoSetColorF(x + offsetDst, f.Invoke(rowSrc.DoGetColorF(x + offsetSrc), rowDst.DoGetColorF(x + offsetDst)));
            }
        ...
        private void PerformCombineWithDithering(IDitheringSession ditheringSession, Func<Color32, Color32, Color32> combineFunction)
        {
            ...
            if (ditheringSession.IsSequential || SourceRectangle.Width < parallelThreshold >> ditheringScale)
            {
                ...
                for (int y = 0; y < SourceRectangle.Height; y++)
                {
                    ...
                    // we can pass x, y to the dithering session because if there is an offset it was initialized by a properly clipped rectangle
                    for (int x = 0; x < SourceRectangle.Width; x++)
                        rowDst.DoSetColor32(x + TargetRectangle.X, ditheringSession.GetDitheredColor(combineFunction.Invoke(rowSrc.DoGetColor32(x + SourceRectangle.X), rowDst.DoGetColor32(x + TargetRectangle.X)), x, y));
            ...
            void ProcessRow(int y)
            {
                ...
                // we can pass x, y to the dithering session because if there is an offset it was initialized by a properly clipped rectangle
                for (int x = 0; x < width; x++)
                    rowDst.DoSetColor32(x + offsetDst, session.GetDitheredColor(f.Invoke(rowSrc.DoGetColor32(x + offsetSrc), rowDst.DoGetColor32(x + offsetDst)), x, y));
            }
